import requests

def get_game_id(game_name):
    """ゲーム名からゲームIDを取得する"""
    base_url = "https://www.speedrun.com/api/v1/games"
    params = {"name": game_name}
    response = requests.get(base_url, params=params)
    data = response.json()
    
    if 'data' in data and data['data']:
        return data['data'][0]['id']
    return None

def get_category_ids(game_id):
    """ゲームIDからカテゴリIDを取得する"""
    base_url = f"https://www.speedrun.com/api/v1/games/{game_id}/categories"
    response = requests.get(base_url)
    data = response.json()

    if 'data' in data:
        category_ids = [category['id'] for category in data['data']]
        return category_ids
    return []

def get_run_details(game_id, category_id):
    """ゲームIDとカテゴリIDからランの詳細を取得する"""
    base_url = f"https://www.speedrun.com/api/v1/leaderboards/{game_id}/category/{category_id}"
    response = requests.get(base_url)
    data = response.json()

    if 'data' in data and 'runs' in data['data']:
        return data['data']['runs']
    return []

def get_video_links(runs):
    """ランの詳細から動画リンクを抽出する"""
    video_links = []

    for run in runs:
        if 'videos' in run['run'] and run['run']['videos'] is not None and 'links' in run['run']['videos']:
            for video in run['run']['videos']['links']:
                if 'uri' in video:
                    video_link = video['uri']
                    video_links.append(video_link)

    return video_links

def main():
    game_name = input("ゲーム名を入力してください: ")
    game_id = get_game_id(game_name)

    if not game_id:
        print(f"{game_name}のゲームIDが見つかりませんでした。")
        return

    category_ids = get_category_ids(game_id)
    all_runs = []

    for category_id in category_ids:
        runs = get_run_details(game_id, category_id)
        all_runs.extend(runs)

    video_links = get_video_links(all_runs)

    file_name = f"{game_name.replace(' ', '_').lower()}_speedrun_links.txt"
    with open(file_name, "w") as file:
        for link in video_links:
            file.write(link + "\n")

    print(f"{len(video_links)}件のユニークな動画リンクが'{file_name}'に記録されました。")

if __name__ == "__main__":
    main()
